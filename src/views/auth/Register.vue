      ...
      <div class="flex-1 flex items-center justify-center p-6 sm:p-12">
        <div class="w-full max-w-sm space-y-8 mt-12 lg:mt-0">
          ...
          <div class="text-center space-y-2">
            <h1 class="text-2xl font-semibold tracking-tight text-zinc-900">Create an account</h1>
            <p class="text-sm text-zinc-500">Start your journey with Resonote.</p>
          </div>

          <!-- Custom Tabs -->
          <div class="grid grid-cols-2 p-1 bg-zinc-50 rounded-xl border border-zinc-100">
            <button @click="goLogin" class="text-sm font-medium py-2 rounded-lg text-zinc-500 hover:text-zinc-700 transition-all">Sign In</button>
            <button class="text-sm font-medium py-2 rounded-lg bg-white text-zinc-900 shadow-sm border border-zinc-100 transition-all">Create Account</button>
          </div>
          ...
          <form class="space-y-4 tab-content" @submit.prevent="onSubmit">
            ...
            <div class="space-y-1 input-group">
              <label class="text-xs font-medium text-zinc-500 ml-1 transition-colors">Password</label>
              <div class="relative group">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            ...
            <div class="space-y-1 input-group">
              <label class="text-xs font-medium text-zinc-500 ml-1 transition-colors">Confirm Password</label>
              <div class="relative group">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <Icon icon="solar:lock-keyhole-linear" class="text-zinc-400 transition-colors group-focus-within:text-zinc-900" width="20" />
                </div>
...
const onSubmit = async () => {
  ...
  try {
    ...
    if (!res.data || (res.data.code !== 200 && res.data.code !== 201)) {
      message.error(res.data?.msg || 'Registration failed')
      return
    }
